...
FILE_HEADER_TO_SAVE = ["time", "device_number", "OryConc", "AV", "CPUTemp", "SensorTemp", "packet_id"]

DATA_PACKET_KEYS = ["time", "device_number", "CPUTemp", "SensorTemp", "packet_id"]
SAVED_DATA_KEYS = DATA_PACKET_KEYS.extend(["AV", "OryConc"])
RAW_DATA_HEADERS = ["time", "device_number", "packet_id"]
...
class DataPacket:
    def __init__(self, date=None, time=None, packet_id=None,
        ...
        if packet:
            ...
            self.device = packet["device_number"]
    ...
    def parse_line(self, csv_line):
        ...
        self.device = split_line[indices["device_number"]]
...
class DeviceData:
    ...
    def add_data_pkt(self, data_pkt, models):
        ...
        if "device_number" in data_pkt:  # this is the code in the sensors still
            position = data_pkt["device_number"].strip()
        elif "position" in data_pkt:  # trying to move all code to here
            position = data_pkt["position"]
        else:
            raise KeyError("data packet has to have 'position' or 'device_number' in it")
...
class TimeStreamData:
    ...
    def add_device(self, position):
        # logging.info(f"adding device_number: {position} to data_class")
        # print(f"adding device_number: {position} to data_class")
    ...
    def add_data(self, data_pkt: dict, save_data=True):
        ...
        if "device_number" in data_pkt:
            ...
            print("No 'device_number' or 'position' in data_pkt")
            return 200
        ...
        if save_data:  # this is live data
            ...
            # self.master_graph.update_temp(device_number, temp, "CPU")
    ...
    def check_missing_packets(self, device, pkts_sent):
        ...
        device_data = self.positions[device]
        print(f"device_number packets: {len(device_data.packet_ids)}, {device_data.packet_ids}")
    ...
    def get_missing_packets_deprecated(self, device):
        ...
        return missing_ids
        # self.check_prev_reads(device_number, self.devices[device_number].latest_packet_id)

    def check_prev_reads_dep(self, device, num_packets):
        print(f"Updating packets for {num_packets} packets")
        # print(f"{self.devices}")
        # print(f"for device_number: {device_number}")
        # print(f"we have packets: {self.devices[device_number].packet_ids}")

        for i in range(1, num_packets + 1):
            # print(f"Comparing {i} in {self.devices[device_number].packet_ids}")
            # print(f"{type(i)} {type(self.devices[device_number].packet_ids)} {type(self.devices[device_number].packet_ids[0])}")
    ...
    def update_latest_packet_id(self, device, pkt_num):
        # this is the first method to see the device_number so add it to devices
        if device not in self.positions:
            self.add_device(device)
        self.positions[device].latest_packet_id = pkt_num

    # def add_mqtt_data(self, data_pkt):
    #     device_number = data_pkt["device_number"]

    def add_db_data(self, data_pkt):
        device = data_pkt["Info"]["device_number"]
        # if device_number is not seen yet, add it to device_number dict
        if device not in self.positions:
            self.add_device(device)
            # self.devices[device_number] = DeviceData()
