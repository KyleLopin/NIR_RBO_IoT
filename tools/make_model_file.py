...
ALL_WAVELENGTHS = [i for i in range(1350, 1651)]
ZEROS = [0 for i in range(1350, 1651)]
WINDOW_WAVELENGTHS = [i for i in range(1360, 1641)]


def view_file():
    """ View each """
    with open("old_models.json", 'r') as _file:
...
def get_old_model_values(device, key):
    with open("old_models.json", 'r') as _file:
        json_data = json.load(_file)
    return json_data[device][key]


def get_ref_values(device) -> list:
    data = pd.read_excel("Reference Values.xlsx",
                         sheet_name=SHEETNAME_MAP[device])
    return data["Intensity"].values.tolist()


def get_coeffs_into_list(df, use_filter=False) -> list:
    """
    ...
    Args:
        df (pd.DataFrame): data frame with wavelengths in the first column
    ...
    for wavelength in wavelengths:
        ...
        if wavelength in df.index:
            coeffs.append(df.loc[wavelength, "coeffs"])
        else:
            coeffs.append(0)
    return coeffs


def make_model_dict(device, model):
    new_model = {}
    ...
    # Get coeffs, need to know if a SG filter is used
    use_filter = False
...
def make_new_file():
    ...
    for device in MODELS.keys():
        ...
        # new_file[device]["Ref Intensities"] = get_old_model_values(device, "Ref Intensities")
    ...
    print(new_file)
    with open("sensor_models.json", 'w') as _file:
        _file.write(json.dumps(new_file, sort_keys=True, indent=4))


def view_new_file():
    with open("sensor_models.json", 'r') as _file:
        json_data = json.load(_file)
    for key in json_data.keys():
        print(key)
        for key2 in json_data[key].keys():
